...
def GetAnomaly(x,axis=-1):
	...
	xt= AxRoll(x,axis)
	#compute anomalies. Use nanmean so partial missing longitudes do not
	#contaminate the whole zonal anomaly field.
	xt = xt - np.nanmean(xt,axis=0)[np.newaxis,:]
...
def ComputeVertEddy(v,t,p,p0=1e3,wave=-1):
	...
	dthdp = np.gradient(t_bar,axis=1,edge_order=2)/dp # dthdp = d(theta_bar)/dp
...
def ComputeEPfluxDiv(lat,pres,u,v,t,w=None,do_ubar=False,wave=-1):
	...
	if do_ubar:
		...
		fhat = R*np.gradient(ubar*coslat,axis=-1,edge_order=2)/dphi
	else:
		fhat = 0.
	fhat = f - fhat # [1/s]
	#
	...
	if do_ubar:
		shear = np.gradient(ubar,axis=1,edge_order=2)/dp # [m/s.hPa]
	else:
		shear = 0.
	...
	div1 = coslat*np.gradient(ep1_cart,axis=-1,edge_order=2)/dphi - 2*sinlat*ep1_cart
	...
	div2 = np.gradient(ep2_cart,axis=1,edge_order=2)/dp # [m/s2]
	#
	# convert to m/s/day
	div1 = div1*86400
	div2 = div2*86400
